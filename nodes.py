...
class WanVideoVACES4frames:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "num_frames": ("INT", {"default": 81, "min": 1, "max": 10000, "step": 4, "tooltip": "Number of frames to encode"}),
            "empty_frame_level": ("FLOAT", {"default": 0.5, "min": 0.0, "max": 1.0, "step": 0.01, "tooltip": "White level of empty frame to use"}),
            # Ahora, en lugar de índices fijos, podrías pensar en porcentajes o simplemente eliminarlos si son siempre proporcionales
            # Para la flexibilidad que pides, lo mejor es que los índices se calculen dinámicamente.
            # Podríamos añadir "proporciones" si quisieras control fino sobre la distancia entre ellos.
            },
            "optional": {
                "start_image": ("IMAGE",),
                "first_intermediate_image": ("IMAGE",),
                "second_intermediate_image": ("IMAGE",),
                "end_image": ("IMAGE",),
                "control_images": ("IMAGE",),
                "inpaint_mask": ("MASK", {"tooltip": "Inpaint mask to use for the empty frames"}),
            },
    ...
    RETURN_NAMES = ("images", "masks",)
    FUNCTION = "process"
    CATEGORY = "WanVideoWrapper/Extended"
    DESCRIPTION = "Helper node to create a batch of frames with a start, end, and two intermediate frames for VACE, fully powered! Now with dynamic indexes, in case you're one of those who likes to change the number of frames without messing up!"

    def process(self, num_frames, empty_frame_level,
                start_image=None, first_intermediate_image=None, second_intermediate_image=None, end_image=None,
                control_images=None, inpaint_mask=None):
        
        # ¡Aquí viene la magia! Calculamos los índices de forma dinámica
        # Siempre el primer frame es 0 y el último es num_frames - 1.
        # Las intermedias las distribuimos de forma equitativa.
        start_frame_idx = 0
        end_frame_idx = num_frames - 1

        # Para las intermedias, las repartimos a tercios (más o menos)
        # Aseguramos que los índices no se salgan y que haya al menos 4 frames para que esto tenga sentido.
        if num_frames < 4:
            raise ValueError("You need at least 4 frames to put a start image, two intermediate ones and a final one, dude!")

        # Calculamos los puntos de inserción. Por ejemplo, al 1/3 y 2/3 del total de frames
        # Usamos round para asegurarnos de que sean enteros.
        first_inter_frame_idx = round(num_frames / 3)
        second_inter_frame_idx = round(num_frames * 2 / 3)

        # Pequeño ajuste para que no se pisen con el inicio/fin si num_frames es muy pequeño
        if first_inter_frame_idx <= start_frame_idx:
            first_inter_frame_idx = start_frame_idx + 1
        if second_inter_frame_idx <= first_inter_frame_idx:
            second_inter_frame_idx = first_inter_frame_idx + 1
        if end_frame_idx <= second_inter_frame_idx:
            end_frame_idx = second_inter_frame_idx + 1 # Esto es importante si num_frames es muy bajo


        # Validación básica para que no se nos monte la feria (aunque con el cálculo dinámico es menos probable, mejor prevenir)
        if not (0 <= start_frame_idx < first_inter_frame_idx < second_inter_frame_idx < end_frame_idx < num_frames):
            # Aquí podríamos ser más específicos si la validación falla por los cálculos dinámicos.
            # Por ahora, un error genérico.
            raise ValueError(f"Hey, the calculated frame indices are either out of order or out of the total number of frames!"
                             f"Calculated values: start={start_frame_idx}, 1st inter={first_inter_frame_idx}, 2nd inter={second_inter_frame_idx}, end={end_frame_idx}, total={num_frames}. Check them, trunk.")

        # Averiguamos las dimensiones y el device de la primera imagen disponible
        ref_image = None
        if start_image is not None: ref_image = start_image
        elif first_intermediate_image is not None: ref_image = first_intermediate_image
        elif second_intermediate_image is not None: ref_image = second_intermediate_image
        elif end_image is not None: ref_image = end_image
            
        if ref_image is None:
            raise ValueError("You need at least one start, middle, or end image to know the dimensions, damn it!")

        # Aseguramos que la imagen de referencia tiene el formato [B, H, W, C]
        if ref_image.dim() == 3: # Si es [H, W, C], le añadimos el batch dimension
            B, H, W, C = 1, ref_image.shape[0], ref_image.shape[1], ref_image.shape[2]
            device = ref_image.device
        else: # Si ya es [B, H, W, C]
            B, H, W, C = ref_image.shape
            device = ref_image.device

        masks = torch.ones((num_frames, H, W), device=device)
        # Inicializamos out_batch_list con tensores de forma [1, H, W, C] para cada frame
        out_batch_list = [torch.ones((1, H, W, 3), device=device) * empty_frame_level for _ in range(num_frames)]

        # Función auxiliar para redimensionar imágenes y asegurar el batch dimension
        def prepare_image_for_insertion(img, target_W, target_H, target_device):
            if img is not None:
                # Si la imagen viene sin batch dimension [H, W, C], lo añadimos
                if img.dim() == 3:
                    img = img.unsqueeze(0) # Convertir a [1, H, W, C]
                
                # Redimensionar si es necesario
                # Nota: common_upscale espera (B, C, H, W) para la interpolación
                if img.shape[1] != target_H or img.shape[2] != target_W: # Formato [B, H, W, C]
                    print(f"Resizing image from {img.shape} to ({target_H}, {target_W}).")
                    img = common_upscale(img.movedim(-1, 1), target_W, target_H, "lanczos", "disabled").movedim(1, -1)
                    
                return img.to(target_device) # Asegurar que está en el mismo device
            return None

        # Colocamos las imágenes si están presentes
        if start_image is not None:
            processed_start_image = prepare_image_for_insertion(start_image, W, H, device)
            if processed_start_image is not None:
                out_batch_list[start_frame_idx] = processed_start_image
                masks[start_frame_idx] = 0

        if first_intermediate_image is not None:
            processed_first_inter_image = prepare_image_for_insertion(first_intermediate_image, W, H, device)
            if processed_first_inter_image is not None:
                out_batch_list[first_inter_frame_idx] = processed_first_inter_image
                masks[first_inter_frame_idx] = 0

        if second_intermediate_image is not None:
            processed_second_inter_image = prepare_image_for_insertion(second_intermediate_image, W, H, device)
            if processed_second_inter_image is not None:
                out_batch_list[second_inter_frame_idx] = processed_second_inter_image
                masks[second_inter_frame_idx] = 0
            
        if end_image is not None:
            processed_end_image = prepare_image_for_insertion(end_image, W, H, device)
            if processed_end_image is not None:
                out_batch_list[end_frame_idx] = processed_end_image
                masks[end_frame_idx] = 0

        # Si tenemos control_images, las usamos para rellenar los huecos
        if control_images is not None:
            # Asegurar que control_images tiene el formato correcto [B, H, W, C] y en el device
            if control_images.dim() == 3:
                control_images = control_images.unsqueeze(0) # Añadir batch dimension si falta
            control_images = common_upscale(control_images.movedim(-1, 1), W, H, "lanczos", "disabled").movedim(1, -1).to(device)

            for i in range(num_frames):
                # Solo rellenamos si el frame está marcado como "vacío" (máscara en 1)
                # y si tenemos una imagen de control disponible para ese índice
                if torch.all(masks[i] == 1) and i < control_images.shape[0]: 
                    # Aseguramos que el frame de control_images se inserte con el batch dimension
                    out_batch_list[i] = control_images[i:i+1] # Slice para mantener el batch dimension
            
        # Finalmente, apilamos todos los tensores. Como todos tienen [1, H, W, C], el resultado será [num_frames, 1, H, W, C]
        # Y luego lo apretamos para que quede [num_frames, H, W, C]
        out_batch = torch.cat(out_batch_list, dim=0) # Usamos cat en vez de stack para unir los batches de 1

        # Aplicación de la máscara de inpaint, esto se mantiene igual
        if inpaint_mask is not None:
            inpaint_mask = common_upscale(inpaint_mask.unsqueeze(1), W, H, "nearest-exact", "disabled").squeeze(1).to(device)
            if inpaint_mask.shape[0] > num_frames:
                inpaint_mask = inpaint_mask[:num_frames]
...
NODE_CLASS_MAPPINGS = {
    ...
    "WanVideoVACES4frames": WanVideoVACES4frames,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "WanVideoVACE4frames": "WanVideo VACE 4 frames",
